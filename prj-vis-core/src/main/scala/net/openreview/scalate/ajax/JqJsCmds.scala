...
object JqJE {
  ...
  case class JqAppend(content: NodeSeq) extends JsExp with JsMember {
    override val toJsCmd = "append("+fixHtmlFunc("inline", content){a => a}+")"
  }
  ...
  case class JqAppendTo(content: NodeSeq) extends JsExp with JsMember {
    override val toJsCmd = "appendTo("+fixHtmlFunc("inline", content){str => str}+ ")"
  }
  ...
  case class JqPrepend(content: NodeSeq) extends JsExp with JsMember {
    override val toJsCmd = "prepend(" + fixHtmlFunc("inline", content){str => str }+ ")"
    // override val toJsCmd = "prepend("+content+")"
  }
  ...
  case class JqPrependTo(content: NodeSeq) extends JsExp with JsMember {
    override val toJsCmd = "prependTo(" + fixHtmlFunc("inline", content){str => str} + ")"
    // override val toJsCmd = "prependTo(" +content+ ")"
  }
  ...
  case class JqEmptyAfter(content: NodeSeq) extends JsExp with JsMember {
    override val toJsCmd = "empty().after(" + fixHtmlFunc("inline", content){str => str} + ")"
    // override val toJsCmd = "empty().after(" +content+ ")"
  }
  ...
  case class JqReplace(content: NodeSeq) extends JsExp with JsMember {
    ...
    // override val toJsCmd = "replaceWith(" + content + ")"
  }
...
object JqJsCmds {
  ...
  object Show {
     ...
     */
    def apply(uid: String) = new Show(uid, None)
     ...
     */
    def apply(uid: String, time: TimeSpan) = new Show(uid, Some(time))
  }
   ...
   * @param time the duration of the effect.
   */
  class Show(val uid: String, val time: Option[TimeSpan]) extends JsCmd with HasTime {
  ...
  object Hide {
     ...
     */
    def apply(uid: String) = new Hide(uid, None)
     ...
     * @param time the duration of the effect.
     */
    def apply(uid: String, time: TimeSpan) = new Hide(uid, Some(time))
  }

  /**
   * Hide an element identified by uid and the animation will last @time
   */
  class Hide(val uid: String, val time: Option[TimeSpan]) extends JsCmd with HasTime {
  ...
  */
  ...
  object ModalDialog {
     ...
     */
    def apply(html: NodeSeq) = new ModalDialog(html, None)
     ...
     */
    def apply(html: NodeSeq, css: JsObj) = new ModalDialog(html, Some(css))
  }

  /**
   * Requires the jQuery blockUI plugin
   */
  class ModalDialog(html: NodeSeq, css: Option[JsObj]) extends JsCmd {
    /*
