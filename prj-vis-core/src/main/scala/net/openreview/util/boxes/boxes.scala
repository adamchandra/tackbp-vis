package net.openreview.util.boxes
...
object BoxesPlayground {
  import net.openreview.util.console._
  import Boxes._
  ...
  def main(args: Array[String]) {
    ...
    val iLoop = new ScalaILoop {
      ...
      override def prompt = "boxes> "
      
      import LoopCommand.{nullary, cmd}

      val additionalCmds = List(
        cmd("example", "example <1..3>", "a few example boxes", (s:String) =>{
          printBox(sampleBox2)
          ()
        }).withLongHelp(
