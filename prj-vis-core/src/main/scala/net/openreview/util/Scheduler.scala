 ...
 * The schedule methods return a ScheduledFuture object which can be cancelled if necessary
 */
sealed trait Schedule extends Logging {
   ...
   * to Full(200000)
   */
  @volatile var blockingQueueSize: Option[Int] = Some(200000)
  
  @volatile var buildExecutor: () => ThreadPoolExecutor = () => new ThreadPoolExecutor(
    threadPoolSize,
    maxThreadPoolSize,
    60,
    TimeUnit.SECONDS,
