...
// ++  /** @return a Option[date] from a string using the internet format. */
// ++  def boxParseInternetDate(dateString: String): Option[Date] = tryo {
...
// ++  /** @return a Some(date) or a failure if the input couldn't be translated to date (or None if the input is null)*/
// ++  def toDate(in: Any): Option[Date] = {
// ++    try {
// ++      in match {
// ++        case null => None
// ++        case d: Date => Some(d)
// ++        case lng: Long => Some(new Date(lng))
// ++        case lng: Number => Some(new Date(lng.longValue))
// ++        case Nil | None | None | Failure(_, _, _) => None
...
// ++      }
// ++    } catch {
// ++      case e: Exception => // logger.debug("Error parsing date "+in, e); Failure("Bad date: "+in, Some(e), None)
// ++    }
// ++  }
}
