...
object Helpers extends StringHelpers
...
trait StringHelpers {
  ...
  private lazy val _currentTlrMethod: Validation[(String, Throwable), Method]= {
    try {
      val tlr = Class.forName("java.util.concurrent.ThreadLocalRandom")
      tlr.getMethod("current").success[(String, Throwable)]
    } catch {
      case e =>
      Failure(("ThreadLocalRandom is not available.", e))
    }
  }
  private def withRng[T](block: (Random)=>T) = {
    _currentTlrMethod.fold(
      f => {
